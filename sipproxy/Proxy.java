...
import gov.nist.sip.proxy.SQL_connection;
...
public class Proxy implements SipListener  {
    ...
    //new variables
    protected long javaTimeStart;
    protected long javaTimeEnd;
    //
    ...
    */
    ...
    //new code section
    private URI getCallerUri(Request request) {
    	FromHeader from = (FromHeader) 
    			request.getHeader(FromHeader.NAME);
    	Address callerAddress=from.getAddress();
        URI callerSipURI=(SipURI) callerAddress.getURI();
        return callerSipURI;
    }
    
    private URI getCalleeUri(Request request) {
    	ToHeader to = (ToHeader) 
    			request.getHeader(ToHeader.NAME);
    	Address calleeAddress=to.getAddress();
        URI calleeSipURI=(SipURI) calleeAddress.getURI();
        return calleeSipURI;
    }
    ...
    public String getUsername(int user_id) throws SQLException{
		SQL_connection sql_c=new SQL_connection();
		ResultSet RSet= sql_c.SelectFromTable("users","*", "`user_id` = '"+user_id+"'");
		if (RSet==null){
			return null;
		}
		else if (RSet.next()){
...
/*******************************************************************************/
		...
		" (an error code has normally been"+
		    ...
		    "the request is an ACK"+
                ...
                SipURI callerURI = (SipURI) getCallerUri(request);
     	       	SipURI calleeURI = (SipURI) getCalleeUri(request);
     	       	 
     	       	int caller = getUserId(getUsername(callerURI));
     	        int callee = getUserId(getUsername(calleeURI));
     	       
     	        java.util.Date javaDate = new java.util.Date();
     	        javaTimeStart = javaDate.getTime();
     	        java.sql.Timestamp start_time = new java.sql.Timestamp(javaTimeStart); 
     	        //System.out.println("START TIME: "  + start_time.toString());
     	        //CallIdHeader call_ID = (CallIdHeader) request.getHeader(CallIdHeader.NAME);
     	        //String target = call_ID.toString().split(":")[1].split("@")[0];
     	          
     	        BServer.call_start(caller, callee, start_time);
     	        //end of code section
            }
...
/*****************************************************************************/            
	  ...
	  if (request.getMethod().equals(Request.BYE) ) {
		...
		clonedRequest.addHeader(via);
	      ...
	      int callee = getUserId(getUsername(calleeURI));
		  java.util.Date javaDate = new java.util.Date();
          javaTimeEnd = javaDate.getTime();
          java.sql.Timestamp end_time = new java.sql.Timestamp(javaTimeEnd);
          //System.out.println("END TIME: "  + end_time.toString());
          
          //CallIdHeader call_ID = (CallIdHeader) request.getHeader(CallIdHeader.NAME);
          //String target = call_ID.toString().split(":")[1].split("@")[0];
		  BServer.call_end(caller,callee, end_time);
		  long time =(javaTimeEnd-javaTimeStart)/1000;
		  BServer.updateBill(caller, time);
		  ...
		  return;
             ...
             */
             ...
             if (request.getMethod().equals("INVITE") && 
             		( (SipURI) getCallerUri(request)).isSipURI()) {
            	 
       	       	 SipURI callerURI = (SipURI) getCallerUri(request);
       	       	 SipURI calleeURI = (SipURI) getCalleeUri(request);
       	       	 
       	       	 String caller = getUsername(callerURI);
       	         String callee = getUsername(calleeURI);
       	         //maybe check if caller!=callee ?       	     
       	         System.out.println("Caller is " + caller + " and Callee is " + callee);
       	         if (caller.equals(callee)){
       	        	Response response2=messageFactory.createResponse
	          	             (Response.TEMPORARILY_UNAVAILABLE,request);
	          	         if (serverTransaction!=null)
	          	            	 serverTransaction.sendResponse(response2);
	          	         else sipProvider.sendResponse(response2);
	          	         return;
       	         }else{
       	  	          	 //Check for Blocking
		       	         Blocking_Server BlServer = Blocking_Server.getInstance();
		       	         if (BlServer.isBlocked(callee,caller)){
		          	         ...
		          	         return;
		       	        	 ...
		       	        	 if (FServer.isForwarded(callee)!=null){
		       	        		Request newreq;
		     	       			Response response=messageFactory.createResponse
		    	       	             (Response.CALL_IS_BEING_FORWARDED,request);
		     	       			if (serverTransaction!=null)
		     	       				 serverTransaction.sendResponse(response);
		    	       	        else sipProvider.sendResponse(response); 
		     	       			
			     	       		 String final_target = FServer.getFinal(callee);
			 	       			 if (final_target == null) {
			 	       				 ...
			 	       				 //check again for blocking
			 	       				 if (BlServer.isBlocked(final_target,caller)){
			 	         	        	 Response response1=messageFactory.createResponse
			 	            	             (Response.TEMPORARILY_UNAVAILABLE,request);
			 	            	         if (serverTransaction!=null)
			 	            	            	 serverTransaction.sendResponse(response1);
			 	            	         else sipProvider.sendResponse(response1);
			 	            	         return;
			 	         	         }else{
				 	       				 System.out.println("Final callee is " + final_target);	 	       				 	 	       				 
				 	                     calleeURI.setUser(final_target); 
				 	                     System.out.println(calleeURI);
				 	                     Address addr = addressFactory.createAddress(calleeURI);
				 	                     ToHeader to = headerFactory.createToHeader(addr, null);
				 	                     System.out.println("New request");
				 	                     newreq = (Request) request.clone();
				 	                     newreq.setHeader(to);
				 	                     request = newreq;
			 	         	         }
			 	       			} 
